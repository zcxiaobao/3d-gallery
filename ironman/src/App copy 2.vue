...
onMounted(() => {
    ...
    loader.load('/model/ironman-zip-2.glb', gltf => {
        const icon_man = gltf.scene;
        console.log(icon_man);
        let r = 3000;
        const allMesh = [];
        icon_man.traverse(function (child) {
            console.log(child);
            allMesh.push(...getAllMesh(child));
        });
        ...
        gsap.to(icon_man.rotation, { y: Math.PI, duration: 10 });
    });
